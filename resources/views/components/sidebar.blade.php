@php
    ...
    try {
        $redzoneCount = \App\Models\Employee::whereHas('scores', function ($q) {
            $q->selectRaw('SUM(points) as total')
              ->having('total', '<', \App\Models\Setting::getValue('redzone_threshold', 50));
        })->count();
    } catch (\Exception $e) {
        $redzoneCount = 0;
    }
...
<aside id="left-panel"
    ...
    <div class="px-4 pt-5 pb-4 border-b border-slate-100 dark:border-slate-700 shrink-0">
        <div class="flex flex-col items-center text-center gap-1.5">
            <div class="w-14 h-14 rounded-xl bg-pcrm-100 dark:bg-pcrm-900/50 flex items-center justify-center text-pcrm-700 dark:text-pcrm-400 font-bold text-xl">
    ...
    <nav class="flex-1 px-3 py-3 space-y-0.5 overflow-y-auto">
        ...
        {{-- ── TỔNG QUAN ─────────────────────────────── --}}
        <p class="px-3 pb-1.5 pt-1 text-[10px] font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider">Tổng quan</p>
        ...
        {{-- ── NHÂN SỰ ──────────────────────────────── --}}
        <p class="px-3 pb-1.5 pt-4 text-[10px] font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider">Nhân sự</p>
        ...
        {{-- ── THƯỞNG PHẠT ──────────────────────────── --}}
        <p class="px-3 pb-1.5 pt-4 text-[10px] font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider">Thưởng phạt</p>
        ...
        <a href="{{ route('redzone.index') }}"
           class="sidebar-link {{ $isActive(['redzone']) ? 'sidebar-link-active' : 'sidebar-link-inactive' }}">
            ...
            @if($redzoneCount > 0)
                <span class="inline-flex items-center justify-center px-1.5 py-0.5 text-[10px] font-bold rounded-full bg-red-500 text-white">{{ $redzoneCount }}</span>
            @endif
        </a>

        <p class="px-3 pb-1.5 pt-3 text-[10px] font-medium text-slate-300 dark:text-slate-600 uppercase tracking-wider">Danh mục</p>
        ...
        {{-- ── HỆ THỐNG ──────────────────────────────── --}}
        <p class="px-3 pb-1.5 pt-4 text-[10px] font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider">Hệ thống</p>
        ...
        <p class="px-3 pb-1.5 pt-4 text-[10px] font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider">Người dùng</p>
...
<!-- Mobile overlay -->
<div id="mobile-panel-overlay"
     class="hidden fixed inset-0 bg-black/40 z-40 lg:hidden"
     onclick="closeMobilePanel()"></div>
...
<aside id="mobile-panel"
    ...
    <div class="px-4 pt-4 pb-3 border-b border-slate-100 dark:border-slate-700 shrink-0">
        <div class="flex items-center gap-3">
            <div class="w-9 h-9 rounded-xl bg-pcrm-100 dark:bg-pcrm-900/50 flex items-center justify-center text-pcrm-700 dark:text-pcrm-400 font-bold text-sm shrink-0">
                {{ strtoupper(substr(auth()->user()->name, 0, 2)) }}
            </div>
            <div class="min-w-0">
            ...
            <button onclick="closeMobilePanel()" class="ml-auto shrink-0 w-7 h-7 flex items-center justify-center rounded-lg text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700">
    ...
    <nav class="flex-1 px-3 py-3 space-y-0.5 overflow-y-auto">
        ...
        <a href="{{ route('dashboard') }}" class="sidebar-link {{ $isActive(['dashboard']) ? 'sidebar-link-active' : 'sidebar-link-inactive' }}">
        ...
        @can('view-employees')
        <a href="{{ route('employees.index') }}" class="sidebar-link {{ $isActive(['employees']) ? 'sidebar-link-active' : 'sidebar-link-inactive' }}">
            <i class="bi bi-people text-base"></i><span>Nhân viên</span>
        </a>
        @endcan
        @can('view-teams')
        <a href="{{ route('teams.index') }}" class="sidebar-link {{ $isActive(['teams']) ? 'sidebar-link-active' : 'sidebar-link-inactive' }}">
            <i class="bi bi-diagram-3 text-base"></i><span>Đội nhóm</span>
        </a>
        @endcan
        @can('view-branches')
        <a href="{{ route('branches.index') }}" class="sidebar-link {{ $isActive(['branches']) ? 'sidebar-link-active' : 'sidebar-link-inactive' }}">
        ...
        @can('view-penalties')
        <a href="{{ route('penalties.index') }}" class="sidebar-link {{ $isActive(['penalties']) ? 'sidebar-link-active' : 'sidebar-link-inactive' }}">
            <i class="bi bi-hammer text-base"></i><span>Xử phạt</span>
        </a>
        @endcan
        <a href="{{ route('rankings.index') }}" class="sidebar-link {{ $isActive(['rankings']) ? 'sidebar-link-active' : 'sidebar-link-inactive' }}">
            <i class="bi bi-trophy text-base"></i><span>Bảng xếp hạng</span>
        </a>
        <a href="{{ route('redzone.index') }}" class="sidebar-link {{ $isActive(['redzone']) ? 'sidebar-link-active' : 'sidebar-link-inactive' }}">
            <i class="bi bi-exclamation-octagon text-base"></i>
            <span class="flex-1">Redzone</span>
            @if($redzoneCount > 0)
                <span class="inline-flex items-center justify-center px-1.5 py-0.5 text-[10px] font-bold rounded-full bg-red-500 text-white">{{ $redzoneCount }}</span>
        ...
        @can('view-violations')
        <a href="{{ route('violations.index') }}" class="sidebar-link {{ $isActive(['violations']) ? 'sidebar-link-active' : 'sidebar-link-inactive' }}">
            <i class="bi bi-book text-base"></i><span>Vi phạm</span>
        </a>
        @endcan
        <a href="{{ route('regulations.index') }}" class="sidebar-link {{ $isActive(['regulations']) ? 'sidebar-link-active' : 'sidebar-link-inactive' }}">
        ...
        <a href="{{ route('settings.index') }}" class="sidebar-link {{ $isActive(['settings']) ? 'sidebar-link-active' : 'sidebar-link-inactive' }}">
            <i class="bi bi-gear text-base"></i><span>Cài đặt</span>
        </a>
        @can('view-activity-log')
        <a href="{{ route('activity.log') }}" class="sidebar-link {{ $isActive(['activity']) ? 'sidebar-link-active' : 'sidebar-link-inactive' }}">
        ...
        @can('manage-users')
        <a href="{{ route('users.index') }}" class="sidebar-link {{ $isActive(['users']) ? 'sidebar-link-active' : 'sidebar-link-inactive' }}">
            <i class="bi bi-people text-base"></i><span>Tài khoản</span>
        </a>
        @endcan
        @can('manage-roles')
        <a href="{{ route('roles.index') }}" class="sidebar-link {{ $isActive(['roles']) ? 'sidebar-link-active' : 'sidebar-link-inactive' }}">
