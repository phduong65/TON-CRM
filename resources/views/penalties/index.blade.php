...
@section('content')
            ...
            <form action="{{ route('penalties.index') }}" method="GET" class="flex flex-wrap gap-2 items-end">
                <div>
                    ...
                    <input type="text" name="search" value="{{ request('search') }}"
                           class="form-input h-9 text-sm w-48" placeholder="Tên NV, mã phiếu...">
                </div>
                <div>
                ...
                <div class="flex items-end gap-2">
                    ...
                    <a href="{{ route('penalties.index') }}" class="btn-secondary h-9 px-4 text-sm inline-flex items-center gap-1">
    ...
    <div class="card">
        <div class="py-16 text-center text-slate-400 dark:text-slate-500">
            <i class="ph-gavel text-4xl mb-3 block"></i>
            ...
            <button onclick="openModal('createPenaltyModal')"
                ...
                <i class="ph-plus-circle"></i> Tạo phiếu đầu tiên
            </button>
            @endcan
        </div>
    </div>
            ...
            <div class="px-5 py-4 flex items-start gap-4">
                ...
                <div class="flex-1 min-w-0">
                    <div class="flex items-center gap-2 mb-1 flex-wrap">
                        ...
                        @endif
                        <span class="{{ $badgeCls }} text-xs bg-white text-[#6ee7b7] dark:text-[#6ee7b7] px-2.5 rounded-full font-medium sm:ml-0">{{ $badgeLbl }}</span>
                    </div>
                ...
                <div class="shrink-0 text-right flex flex-col items-end gap-1.5">
                    ...
                    <div class="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity"
                        ...
                        <button type="button"
                                title="Sửa"
                                onclick="openEditPenaltyModal({{ $penalty->id }}, {{ $penalty->violation_id }}, {{ $penalty->employee_id }}, {{ $penalty->total_points_deducted }}, {{ $penalty->total_money_deducted }}, '{{ addslashes($penalty->description ?? '') }}')"
