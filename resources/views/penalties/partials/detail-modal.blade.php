        ...
        <div class="flex-1 overflow-y-auto px-6 py-5 space-y-5">
            ...
            <div id="detail-body-content" class="hidden space-y-5">
                ...
                <div class="grid grid-cols-2 gap-x-6 gap-y-3">
                    <div>
                        <p class="text-[10px] font-semibold uppercase tracking-wider text-slate-400 dark:text-slate-500 mb-0.5">Nhân viên vi phạm</p>
                    ...
                    <div>
                        <p class="text-[10px] font-semibold uppercase tracking-wider text-slate-400 dark:text-slate-500 mb-0.5">Vi phạm</p>
                        <p id="detail-violation-name" class="text-sm font-medium text-slate-800 dark:text-slate-200"></p>
                ...
                <div class="grid grid-cols-2 gap-x-6 gap-y-3">
                    <div>
                        <p class="text-[10px] font-semibold uppercase tracking-wider text-slate-400 dark:text-slate-500 mb-0.5">Điểm trừ</p>
                        <p id="detail-points"
                           class="text-2xl font-bold text-red-600 dark:text-red-400 leading-none"></p>
                    </div>
                    <div id="detail-money-wrap" class="hidden">
                        <p class="text-[10px] font-semibold uppercase tracking-wider text-slate-400 dark:text-slate-500 mb-0.5">Tiền phạt</p>
                        <p id="detail-money"
                           class="text-2xl font-bold text-red-600 dark:text-red-400 leading-none"></p>
                    </div>
                </div>
                ...
                <div id="detail-desc-wrap" class="hidden">
                    <p class="text-[10px] font-semibold uppercase tracking-wider text-slate-400 dark:text-slate-500 mb-1">Mô tả / Ghi chú</p>
                ...
                <div id="detail-members-wrap" class="hidden">
                    <p class="text-[10px] font-semibold uppercase tracking-wider text-slate-400 dark:text-slate-500 mb-2">Thành viên liên đới</p>
                ...
                <div id="detail-attachments-wrap" class="hidden">
                    <p class="text-[10px] font-semibold uppercase tracking-wider text-slate-400 dark:text-slate-500 mb-2">Tệp đính kèm</p>
                ...
                <div id="detail-approved-wrap"
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <div>
                            <p class="text-xs font-semibold text-emerald-700 dark:text-emerald-400">Đã duyệt bởi <span id="detail-approver-name"></span></p>
        ...
        <div class="shrink-0 px-6 py-4 border-t border-slate-200 dark:border-slate-700">
            ...
            <div id="detail-footer-content" class="hidden flex items-center gap-2 flex-wrap">
                ...
                <div id="detail-pending-actions" class="hidden flex items-center gap-2 ml-auto flex-wrap">
                    <button id="detail-edit-btn" onclick="openEditFromDetail()"
...
(function () {
    ...
    /* ── helpers ── */
    function show(id)  { var el = document.getElementById(id); if (el) { el.classList.remove('hidden'); el.classList.add('flex'); } }
    function hide(id)  { var el = document.getElementById(id); if (el) { el.classList.add('hidden'); el.classList.remove('flex'); } }
    ...
    function _populate(d) {
            ...
            if (d.members && d.members.length > 0) {
                ...
                membersList.innerHTML = d.members.map(function (m) {
                    return '<div class="flex items-center justify-between px-3 py-2.5 text-sm">'
                        ...
                        + '</div>'
                        + '<span class="font-semibold text-red-600 dark:text-red-400 shrink-0 ml-4">-' + m.points_deducted + 'đ</span>'
                        + '</div>';
                }).join('');
            } else {
                ...
                attGrid.innerHTML = d.attachments.map(function (a) {
                    if (a.type === 'image') {
                        return '<a href="' + a.url + '" target="_blank"'
                            + '   class="group relative aspect-square rounded-lg overflow-hidden border border-slate-200 dark:border-slate-700 bg-slate-100 dark:bg-slate-800 block">'
                            + '  <img src="' + a.url + '" alt="' + a.filename + '" class="w-full h-full object-cover transition-transform group-hover:scale-105">'
                    ...
                    return '<a href="' + a.url + '" target="_blank"'
                        ...
                        + '  <p class="text-[10px] text-slate-400 text-center leading-tight break-all line-clamp-2">' + a.filename + '</p>'
                        + '  <p class="text-[9px] text-slate-300 dark:text-slate-500">' + a.size + '</p>'
                        + '</a>';
                }).join('');
            } else {
        ...
        if (d.status === 'approved')  { text('detail-approver-name', d.approver); text('detail-approved-at', d.approved_at); }
        if (d.status === 'rejected')  { text('detail-rejected-reason', d.rejected_reason); }
